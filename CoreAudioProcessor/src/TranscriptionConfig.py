...
class TranscriptionConfig():
  def __init__(self):
    ...
    self._application_file = self.command_line_args[CONST.INFILE_KEY]
    ...
    #
    self._performance_log_name = str(Path(self.get(CONST.LOGFILE_DIR_KEY, config_dict), self.get(CONST.PERFORMANCE_LOG_NAME_KEY)))

  def get(self, key, config_dict=DEFAULT.TRANSCRIBER_CONFIG):
    """
  ...
  def input_name(self):            return str(Path(self._application_file).absolute())
  def performance_log_name(self):  return self._performance_log_name
