...
try:
  ...
  error_message = STATUS.transcript_errmsg( err_msg )
...
make_elt = lambda key, value: f"<{str(key).lower()}/>" if value is None else f"<{str(key).lower()}>{str(value).strip(' ')}</{str(key).lower()}>"
  ...
  def asXMLcontent( stuff ):
    ...
    if (len(stuff) == 2) and isinstance(stuff[0], (str, int)):
      if isinstance(stuff[1], (str, int, type(None))):
         return make_elt( stuff[0], None if stuff[1] is None else str(stuff[1]).replace('&','&amp;').replace('<','&lt;'))
    ...
    error_message = STATUS.gradapp_errmsg( err_msg )
...
class XMLFile( object ):
  def __init__( self, config_file=DEFAULT.DEFAULT_CONFIG_FILE, config_schema=DEFAULT.DEFAULT_CONFIG_FILE_SCHEMA ):
    #
    self._contents = {}
    self.file_missing = []
    self.file_malformed = []
