# *************************************************************************************************************************
#   LogWrapper.py
#     A configurable wrapper for the Python logging module, supporting console and file logging with optional
...
class LogWrapper(object):
    ...
    def __init__(self, console_name, console_log_level, console_colorize, logfile_dir, logfile_name, logfile_log_level, logfile_colorize, record_format=DEFAULT.LOG_RECORD_FORMAT_CONFIG):
    ...
    def set_up_logging(self):
        ...
        if (self.logfile_dir == "stdout"):
          logfile_handler = logging.FileHandler(sys.stdout)
        elif (self.logfile_dir == "stderr"):
          logfile_handler = logging.FileHandler(sys.stderr)
        elif (self.logfile_dir == "null"):
          logfile_handler = logging.FileHandler(os.devnull)
        else:
          logfile_dir_absolute_path = Path( self.logfile_dir ).absolute()
          logfile_name = Path( logfile_dir_absolute_path, self.logfile_name ).absolute()
          try:
