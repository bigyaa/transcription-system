...
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
#
# err_to_str - return string in an exception object, if any - else, ''
# statusmsg  - format status message, prefixing it with thread name and time
# errsmsg    - format error message,  prefixing with thread name and time
...
class ExitStatus(object):
  @staticmethod
  def transcript_format_irregularity():       
  ...
  def missing_default_file():                  
    ...
    return ExitStatus.status
  @staticmethod
  def whisperX_speaker_error():                  
    ExitStatus.status = max(WARNING, ExitStatus.status)
    return ExitStatus.status
  @staticmethod
  def uncertain_error():                    
    ExitStatus.status = max(UNCERTAIN_ERROR, ExitStatus.status)
    return ExitStatus.status
  @staticmethod
