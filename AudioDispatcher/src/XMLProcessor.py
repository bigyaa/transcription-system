...
#   DISPATCHER_CONFIG - XML file default values
...
make_elt = lambda key, value: (
    f"<{str(key).lower()}/>"
    if value is None
    else f"{open_tag(key)}{str(value).strip(' ')}{close_tag(key)}"
)
...
# asXML proper
# --------------
#  Note: the prettyprint option allows for suppressing the involvement of MINIDOM.parseString, which adds an XML declaration
#  to its output
#---------------------------------------------------------------------------------------------------------------------------

def asXML(outer_tag, content, logger, prettyprint=True):
    ...
    content = make_elt( outer_tag, asXMLcontent(content))
    return prettyPrint( content ) if prettyprint else content
...
class XMLFile(object):
    def __init__( self, config_file=DEFAULT.DISPATCHER_CONFIG[CONST.CONFIG_FILE_KEY], config_schema=DEFAULT.DISPATCHER_CONFIG[CONST.CONFIG_SCHEMA_KEY] ):
        #
        self._contents = {}
        self.file_missing = []
        self.file_malformed = []
